local function UpdateBtnPos(ctrl)
    ...
    local bkg = ctrl:GetControlObject("sliderbar.bkg")   
...
function Btn__OnMinus(self)
	local ctrl = self:GetOwnerControl()
  local attr = ctrl:GetAttribute()
  local newPos=attr.Pos - attr.LargeChange

	if newPos < attr.Min then
  	newPos = attr.Min
  elseif newPos > attr.Max then
   	newPos = attr.Max
  end
...
function Btn__OnPlus(self)
	local ctrl = self:GetOwnerControl()
  local attr = ctrl:GetAttribute()
  attr.Pos=attr.Pos + attr.LargeChange	
	UpdatePos(ctrl)
end